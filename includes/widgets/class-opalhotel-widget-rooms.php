...
class OpalHotel_Widget_Rooms extends WP_Widget {

	/* widget base ID */
	public $id = 'opalhotel-rooms';
	...
	function __construct() {
		parent::__construct(
			$this->id,
			__( 'OpalHotel: Rooms', 'opal-hotel-room-booking' ),
			array( 'description' => __( 'Rooms widget', 'opal-hotel-room-booking' ), ) // Args
		);
	}
	...
	public function widget( $args, $instance ) {
		...
		if ( isset( $opalhotel_shortcodes['rooms'] ) ) {
			$opalhotel_shortcodes['rooms']->render( $instance );
		}
		...
		?>
			...
			<p>
				...
				<select class="widefat opalhotel-widget-input" name="<?php echo $this->get_field_name( 'layout' ); ?>">
					...
					<option value="list" <?php selected( $layout, 'overlap' ); ?>><?php esc_html_e( 'Overlap', 'opal-hotel-room-booking' ); ?></option>
				</select>
			</p>
			<p>
